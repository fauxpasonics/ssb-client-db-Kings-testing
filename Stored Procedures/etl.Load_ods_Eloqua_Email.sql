...
SELECT CAST(NULL AS BINARY(32)) ETL_DeltaHashKey, A.*
INTO #SrcData
FROM (
	SELECT ETL_UpdatedDate, ETL_IsDeleted, ETL_DeletedDate, ID, Name, EmailGroupId, HtmlContent, IsPlainTextEditable, IsTracked, PlainText, BouncebackEmail, ReplyToEmail, ReplyToName, SenderEmail, SenderName, SenderPlainTextOnly, Subject, Description, FolderId, Permissions, CreatedAt, CreatedBy, AccessedAt, CurrentStatus, Depth, UpdatedAt, UpdatedBy,
		ROW_NUMBER() OVER (PARTITION BY ID ORDER BY ETL_ID) AS RowNum 
