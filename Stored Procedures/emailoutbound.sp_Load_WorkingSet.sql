...
BEGIN TRY
		...
		INTO #prod
		FROM ods.Eloqua_Contact (NOLOCK)

		CREATE INDEX IX_emailaddress ON #prod (Emailaddress)
		...
		FROM dbo.FactTicketSales fts (NOLOCK)
			...
			LEFT JOIN #prod prod ON prod.EmailAddress = dc.EmailPrimary
		WHERE prod.EmailAddress IS NULL
		...
		SELECT pc.email, 'CRM_Contacts' , MIN(pc.LoadDate) CreatedDate
		FROM KingsCI_Reporting.prodcopy.Contact pc (NOLOCK)
			JOIN dbo.DimCustomer dc (NOLOCK) ON dc.SSID = pc.id
			LEFT JOIN #prod prod ON prod.EmailAddress = dc.EmailPrimary
		WHERE prod.EmailAddress IS NULL
			  AND dc.SourceSystem = 'CRM_Contact'
			  AND dc.EmailPrimaryIsCleanStatus LIKE 'valid%'
		GROUP BY pc.email
		...
		----============================
		----INSERT INTO #Inserts (EmailPrimary,RecordSource,CreatedDate)
		----SELECT * FROM KingsCI_Reporting.prodcopy.Account a
		----WHERE 
		----SELECT pa.emailaddress1, 'CRM_Account' , MIN(pa.CopyLoadDate) CreatedDate
		----FROM KingsCI_Reporting.prodcopy.Account pa (NOLOCK)
		----	JOIN dbo.DimCustomer dc (NOLOCK) ON dc.SSID = pa.id
		----	LEFT JOIN #prod prod ON prod.email = dc.EmailPrimary
		----WHERE prod.Email IS NULL
		----	  AND dc.SourceSystem = 'CRM_Account'
		----	  AND dc.EmailPrimaryIsCleanStatus LIKE 'valid%'
		...
		SELECT Host.email, 'Host' , MIN(host.ETL__CreatedDate) CreatedDate
		FROM ods.Host_Accounts Host (NOLOCK)
			JOIN dbo.DimCustomer dc (NOLOCK) ON dc.SSID = host.HostAccountId
			LEFT JOIN #prod prod ON prod.EmailAddress = dc.EmailPrimary
		WHERE prod.EmailAddress IS NULL
			  AND dc.SourceSystem = 'Host'
			  AND dc.EmailPrimaryIsCleanStatus LIKE 'valid%'
		GROUP BY Host.email
		...
		SELECT LA.cust_email_addr, 'LA' , MIN(LA.ETL_CreatedDate) CreatedDate
		FROM ods.LiveAnalytics_Customer LA (NOLOCK)
			JOIN dbo.DimCustomer dc (NOLOCK) ON dc.SSID = LA.ult_party_id 
			LEFT JOIN #prod prod ON prod.EmailAddress = dc.EmailPrimary
		WHERE prod.EmailAddress IS NULL
			  AND dc.SourceSystem = 'LA_Cust'
			  AND dc.EmailPrimaryIsCleanStatus LIKE 'valid%'
		GROUP BY LA.cust_email_addr
		...
		FROM (
				...
				FROM #Inserts inserts
					JOIN ( SELECT 'TM'			RecordSource	, 1 SourceRank 	UNION ALL
						   SELECT 'Host'    	RecordSource	, 2 SourceRank	UNION ALL
						   SELECT 'CRM_Account'	RecordSource	, 3 SourceRank	UNION ALL
						   SELECT 'CRM_Contact'	RecordSource	, 4 SourceRank 	UNION ALL
						   SELECT 'LA_Cust'  	RecordSource	, 5 SourceRank	UNION ALL
						   SELECT 'Skidata'	    RecordSource	, 6 SourceRank	UNION ALL
						-- SELECT 'Experience'	RecordSource	, 7 SourceRank	UNION ALL
						   SELECT 'Appetize'    RecordSource	, 8 SourceRank				
