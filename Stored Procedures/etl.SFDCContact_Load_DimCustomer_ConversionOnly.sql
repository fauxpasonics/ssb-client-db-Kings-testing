...
CREATE PROCEDURE [etl].[SFDCContact_Load_DimCustomer_ConversionOnly]
...
-- DROP TABLE #tmpETL
SELECT * INTO #tmpETL FROM etl.[vw_SFDCContact_Load_DimCustomer_ConversionOnly] --where SSID = '00380000023rtGMAAY'
...
(
		SourceDB, SourceSystem, SourceSystemPriority, SSID, NameDirtyHash, NameIsCleanStatus, FullName, CompanyName, Prefix, FirstName, LastName
		/*, CustomerType, CustomerStatus, AccountType, AccountRep, , Birthday, Gender, MergedRecordFlag, MergedIntoSSID, Prefix, FirstName, MiddleName, LastName, Suffix*/
...
THEN UPDATE SET 
	...
	UPDATE A SET
	 ...
	 FullName = CASE WHEN a.isbusiness = 1 THEN a.FullName Else ISNULL(b.FullName,a.FullName) END
	, Prefix = CASE WHEN a.isbusiness = 1 THEN a.Prefix ELSE ISNULL(b.Prefix,a.Prefix) END
	, FirstName = CASE WHEN a.isbusiness = 1 THEN a.FirstName ELSE ISNULL(b.FirstName,a.FirstName) END
	, LastName = CASE WHEN a.isbusiness = 1 THEN a.LastName ELSE ISNULL(b.LastName,a.LastName) END
	, NameDirtyHash = CASE WHEN a.isbusiness = 1 THEN a.NameDirtyHash ELSE ISNULL(b.NameDirtyHash,a.NameDirtyHash) END
	 
	, AddressPrimaryDirtyHash = ISNULL(b.AddressPrimaryDirtyHash,a.AddressPrimaryDirtyHash)
	, AddressPrimaryStreet = ISNULL(b.AddressPrimaryStreet,a.AddressPrimaryStreet)
	, AddressPrimaryCity = ISNULL(b.AddressPrimaryCity,a.AddressPrimaryCity)
	, AddressPrimaryState = ISNULL(b.AddressPrimaryState,a.AddressPrimaryState)
	, AddressPrimaryZip = ISNULL(b.AddressPrimaryZip,a.AddressPrimaryZip)
	, AddressPrimaryCounty = ISNULL(b.AddressPrimaryCounty,a.AddressPrimaryCounty)
	, AddressPrimaryCountry = ISNULL(b.AddressPrimaryCountry,a.AddressPrimaryCountry)
	, AddressPrimarySuite = ISNULL(b.AddressPrimarySuite, a.AddressPrimarySuite)

	, PhonePrimaryDirtyHash = CASE WHEN a.isbusiness = 1 THEN a.PhonePrimaryDirtyHash Else ISNULL(b.PhonePrimaryDirtyHash,a.PhonePrimaryDirtyHash) END
	, PhonePrimary = CASE WHEN a.isbusiness = 1 THEN a.PhonePrimary Else ISNULL(b.PhonePrimary,a.PhonePrimary) END

	, EmailPrimaryDirtyHash = CASE WHEN a.isbusiness = 1 THEN a.EmailPrimaryDirtyHash Else ISNULL(b.EmailPrimaryDirtyHash,a.EmailPrimaryDirtyHash) END 
	, EmailPrimary = CASE WHEN a.isbusiness = 1 THEN a.EmailPrimary Else ISNULL(b.EmailPrimary,a.EmailPrimary) END
	, ExtAttribute6 = 'Updated from matching TM record'
	FROM dbo.DimCustomer a
	JOIN dbo.DimCustomer b
		ON CASE WHEN ISNUMERIC(a.ExtAttribute5) = 0 OR a.ExtAttribute5 in ('16544612315646') THEN NULL ELSE a.ExtAttribute5 END = b.AccountId
		AND a.SourceSystem = 'Kings PC_SFDC Contact'
		AND b.CustomerType = 'Primary'
	JOIN dbo.DimCustomer c
		ON a.ExtAttribute5 = c.ExtAttribute5
		AND c.ExtAttribute5 IS NOT NULL
