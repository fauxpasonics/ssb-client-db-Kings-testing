...
THEN UPDATE SET 
	...
	--SELECT SSB_CRMSYSTEM_CONTACT_ID into #SingleAccounts
	--from  dbo.dimcustomerSSBID a join dbo.dimcustomer b on a.dimcustomerid = b.dimcustomerid
	--where SSB_CRMSYSTEM_ACCT_ID is null AND a.SourceSystem = 'TM'
	--group by SSB_CRMSYSTEM_CONTACT_ID
	--having count(distinct b.accountID) = 1

	--Update a
	--Set customer_matchkey = a.AccountID
	--FROM dbo.dimcustomer a join dbo.dimcustomerSSBID b on a.dimcustomerid = b.dimcustomerid
	----JOIN #SingleAccounts c on b.SSB_CRMSYSTEM_CONTACT_ID = c.SSB_CRMSYSTEM_CONTACT_ID
	--WHERE a.SourceSystem = 'TM' AND a.CustomerType ='Primary'
	--AND ISNULL(customer_matchkey,'') <> ISNULL(AccountID,'')
