...
CREATE PROCEDURE [etl].[Load_ods_SkiData_UserBadgesDaily]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount INT = ISNULL((SELECT CONVERT(VARCHAR, COUNT(*)) FROM src.SkiData_UserBadgesDaily),'0');	
...
INTO #SrcData
FROM src.SkiData_UserBadgesDaily
;
...
MERGE ods.SkiData_UserBadgesDaily AS myTarget
...
DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SkiData_UserBadgesDaily WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SkiData_UserBadgesDaily WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
