...
IF EXISTS (SELECT TOP 1 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA IN ('apietl','stg') AND TABLE_TYPE = 'BASE TABLE' AND '[' + TABLE_SCHEMA + '].[' + Table_Name + ']' NOT IN (SELECT '[' + [Schema] + '].[' + [Name] + ']' Name FROM [etl].[JSON_Meta_Table]))
BEGIN

SET @SQL = '
INSERT INTO [etl].[JSON_Meta_Table] ([Schema],Name)
SELECT TABLE_SCHEMA,Table_Name FROM INFORMATION_SCHEMA.Tables WHERE TABLE_SCHEMA IN (''apietl'',''stg'') AND TABLE_TYPE = ''BASE TABLE'' AND ''['' + TABLE_SCHEMA + ''].['' + Table_Name + '']'' NOT IN (SELECT DISTINCT ''['' + [Schema] + ''].['' + [Name] + '']'' FROM [etl].[JSON_Meta_Table])
