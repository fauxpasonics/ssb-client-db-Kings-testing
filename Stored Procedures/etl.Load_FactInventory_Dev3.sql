...
CREATE PROCEDURE [etl].[Load_FactInventory_Dev3]
...
*/
	...
	FROM (
		...
		INNER JOIN dbo.DimEvent de ON avail.event_id = de.SSID_event_id AND de.SourceSystem = 'TM'
		INNER JOIN #EventList el ON de.DimEventId = el.DimEventId
		LEFT OUTER JOIN dbo.DimClassTM dctm ON dctm.ETL_SSID_class_id = avail.class_id --AND dctm.SourceSystem = 'TM'
		LEFT OUTER JOIN dbo.DimPriceCode dpc ON avail.price_code = dpc.PriceCode AND avail.event_id = dpc.SSID_event_id AND dpc.SourceSystem = 'TM'
		INNER JOIN dbo.Lkp_SeatList sl ON sl.seat BETWEEN avail.seat_num AND (avail.seat_num + avail.num_seats - 1)
		...
		INNER JOIN dbo.DimEvent de ON eip.event_id = de.SSID_event_id AND de.SourceSystem = 'TM'
		INNER JOIN #EventList el ON de.DimEventId = el.DimEventId
		LEFT OUTER JOIN dbo.DimClassTM dctm ON dctm.ETL_SSID_class_id = avail.class_id --AND dctm.SourceSystem = 'TM'
		LEFT OUTER JOIN dbo.DimPriceCode dpc ON avail.price_code = dpc.PriceCode AND avail.event_id = dpc.SSID_event_id AND dpc.SourceSystem = 'TM'
		INNER JOIN dbo.Lkp_SeatList sl ON sl.seat BETWEEN avail.seat_num AND (avail.seat_num + avail.num_seats - 1)
...
*/
	...
	FROM (
		...
		INNER JOIN #EventList el ON de.DimEventId = el.DimEventId
		LEFT OUTER JOIN dbo.DimClassTM dctm ON dctm.ETL_SSID_class_id = hs.class_id --AND de.SourceSystem = 'TM'
		LEFT OUTER JOIN dbo.DimPriceCode dpc ON hs.price_code = dpc.PriceCode AND hs.event_id = dpc.SSID_event_id AND dpc.SourceSystem = 'TM'
		INNER JOIN dbo.Lkp_SeatList sl on sl.seat between hs.seat_num and (hs.seat_num + hs.num_seats - 1)
		...
		INNER JOIN dbo.DimEvent de on hs.event_id = de.SSID_event_id AND de.SourceSystem = 'TM'
		INNER JOIN #EventList el ON de.DimEventId = el.DimEventId
		LEFT OUTER JOIN dbo.DimClassTM dctm ON dctm.ETL_SSID_class_id = hs.class_id --AND dctm.SourceSystem = 'TM'
		LEFT OUTER JOIN dbo.DimPriceCode dpc ON hs.price_code = dpc.PriceCode AND hs.event_id = dpc.SSID_event_id AND dpc.SourceSystem = 'TM'
		INNER JOIN dbo.Lkp_SeatList sl on sl.seat between hs.seat_num and (hs.seat_num + hs.num_seats - 1)
		INNER JOIN dbo.DimSeat dSeat on --ds.ManifestId = dSeat.ManifestId and 
			hs.section_id = dSeat.ssid_section_id 
