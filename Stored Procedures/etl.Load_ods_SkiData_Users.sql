...
CREATE PROCEDURE [etl].[Load_ods_SkiData_Users]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.SkiData_Users),'0');	
...
,  ETL_SourceFileName, TicketAccountID, UserID, Username, FirstName, LastName, Email, DisplayName, DeletedUser, CreatedOnDate, LastModifiedOnDate, Authorised, DeletedFromSite
INTO #SrcData
FROM (
	SELECT  ETL_SourceFileName, TicketAccountID, UserID, Username, FirstName, LastName, Email, DisplayName, DeletedUser, CreatedOnDate, LastModifiedOnDate, Authorised, DeletedFromSite
	, ROW_NUMBER() OVER(PARTITION BY UserID ORDER BY ETL_ID) RowRank
	FROM src.SkiData_Users
) a
WHERE RowRank = 1
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(Authorised),'DBNULL_TEXT') + ISNULL(RTRIM(CreatedOnDate),'DBNULL_TEXT') + ISNULL(RTRIM(DeletedFromSite),'DBNULL_TEXT') + ISNULL(RTRIM(DeletedUser),'DBNULL_TEXT') + ISNULL(RTRIM(DisplayName),'DBNULL_TEXT') + ISNULL(RTRIM(Email),'DBNULL_TEXT') + ISNULL(RTRIM(FirstName),'DBNULL_TEXT') + ISNULL(RTRIM(LastModifiedOnDate),'DBNULL_TEXT') + ISNULL(RTRIM(LastName),'DBNULL_TEXT') + ISNULL(RTRIM(TicketAccountID),'DBNULL_TEXT') + ISNULL(RTRIM(UserID),'DBNULL_TEXT') + ISNULL(RTRIM(Username),'DBNULL_TEXT'))
...
MERGE ods.SkiData_Users AS myTarget
USING (
	SELECT * FROM #SrcData
) AS mySource
...
THEN UPDATE SET
     ...
     , myTarget.[DeletedUser] = mySource.[DeletedUser]
     , myTarget.[CreatedOnDate] = mySource.[CreatedOnDate]
     , myTarget.[LastModifiedOnDate] = mySource.[LastModifiedOnDate]
     , myTarget.[Authorised] = mySource.[Authorised]
     , myTarget.[DeletedFromSite] = mySource.[DeletedFromSite]
...
VALUES
     ...
     ,mySource.[DeletedUser]
     ,mySource.[CreatedOnDate]
     ,mySource.[LastModifiedOnDate]
     ,mySource.[Authorised]
     ,mySource.[DeletedFromSite]
     )
;

EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Merge Load', 'Merge Statement Execution', 'Complete', @ExecutionId

DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SkiData_Users WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SkiData_Users WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
