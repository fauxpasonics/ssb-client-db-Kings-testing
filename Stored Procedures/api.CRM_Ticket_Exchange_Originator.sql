...
END
        ...
        SELECT ssbid.SSB_CRMSYSTEM_CONTACT_ID AS SSB_CRMSYSTEM_CONTACT_ID
			  ...
			  ,repname.FirstName+' '+repname.LastName AS O_Rep_Name
		INTO #tmpBase
        FROM    ods.TM_Tex Tex
				...
				LEFT JOIN dbo.DimCustomer repname WITH (NOLOCK) ON repname.AccountId = Tex.assoc_acct_id AND repname.CustomerType = 'Primary' AND repname.SourceSystem = 'TM'
...
, ISNULL(O_Recipient_Account_Id,0) Recipient_Account_Id
, ISNULL(O_Rep_Name, '') Recipient_Name
INTO #tmpOutput
FROM #tmpBase
