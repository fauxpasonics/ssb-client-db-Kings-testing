...
CREATE PROC [ro].[ETL_Eloqua_Contacts_outbound]
AS
BEGIN
...
IF OBJECT_ID('tempdb.Kings.#DimCustomer_ModAcctId_EloquaContact') IS NOT NULL DROP TABLE #DimCustomer_ModAcctId_EloquaContact;


--	For the joins to Eloqua tables below, only consider active/not-deleted contacts
SELECT d.SSB_CRMSYSTEM_CONTACT_ID, d.SourceSystem, d.EmailPrimary, d.SSID
INTO #DimCustomer_ModAcctId_EloquaContact
FROM dbo.vwDimCustomer_ModAcctId d
JOIN ods.eloqua_contact C (nolock)
	ON c.id = d.ssid
	AND d.SourceSystem = 'Eloqua Kings'
	AND c.ETL_IsDeleted = 0
--	GROUP BY d.SSB_CRMSYSTEM_CONTACT_ID, d.SourceSystem, d.EmailPrimary, d.SSID having count(9) > 1
;


--	This is selecting the max date of engagement for each guid and its related sources
--	For all Eloqua source systems, #DimCustomer_ModAcctId_EloquaContact replaced dbo.vwDimCustomer_ModAcctId		DCH 2018-02-22
SELECT *
INTO  #tmpMaxDate
FROM (

SELECT d.SSB_CRMSYSTEM_CONTACT_ID, d.SourceSystem, d.EmailPrimary, MAX(c.createdat) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d
JOIN ods.eloqua_contact C (nolock)
	ON c.id = d.ssid
--	AND d.SourceSystem = 'Eloqua Kings'
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.order_date) AS MaxDate
FROM dbo.vwDimCustomer_ModAcctId d (nolock)
JOIN ods.Appetize_orders a
	ON a.email = d.ssid
	AND d.sourcesystem = 'Appetize'
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary


UNION 

SELECT  d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.ELoqua_ActivityEmailOpen a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityEmailClickThrough a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityEmailSend a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate 
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityEmailSubscribe a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityFormSubmit a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityPageView a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, MAX(a.CreatedAt) AS MaxDate
FROM #DimCustomer_ModAcctId_EloquaContact d 
JOIN ods.Eloqua_ActivityWebVisit a (nolock)
	ON a.contactID = d.ssid
--	AND d.sourcesystem = 'Eloqua Kings'
	AND a.createdat > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.SaleDate) AS MaxDate
FROM dbo.vwDimCustomer_ModAcctId d 
JOIN ods.Host_Trans a (nolock)
	ON a.Calc_HostAccountId = d.AccountId
	AND d.sourcesystem = 'Host Kings'
	AND a.SaleDate > (GETDATE () - 1825)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary


UNION 

SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, max(a.SSCreatedDate) AS MaxDate
FROM dbo.vwDimCustomer_ModAcctId d 
JOIN dbo.FactTicketSales a
	ON a.dimcustomerID = d.DimCustomerId
	AND d.sourcesystem = 'TM'
	AND a.SSCreatedDate > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary


UNION

SELECT d.ssb_crmsystem_contact_ID, d.SourceSystem, d.EmailPrimary, MAX(t.add_datetime) AS MaxDate
FROM dbo.vwDimCustomer_ModAcctId d
JOIN ods.TM_Tex t (nolock)
	ON d.AccountId = CAST(t.acct_ID AS nvarchar) AND d.SourceSystem = 'TM' AND d.CustomerType = 'Primary'
Where t.add_datetime > (GETDATE () - 730)
GROUP BY d.ssb_crmsystem_contact_ID, d.sourcesystem, d.EmailPrimary
...
from
(
	SELECT ssb_crmsystem_contact_ID, SourceSystem, EmailPrimary, MaxDate
		, ROW_NUMBER() OVER (PARTITION BY SSB_CRMSYSTEM_CONTACT_ID,EmailPrimary	ORDER BY MaxDate desc) ma
	FROM #tmpMaxDate
)a
WHERE ma = 1;
...
INTO   #activecontacts
--	FROM #contacts c		changed to #tmpMaxDate DCH 2018-02-22
FROM #tmpMaxDate c
...
SELECT e.*, ROW_NUMBER() OVER(PARTITION BY e.EmailPrimary ORDER BY e.UnsubscribedWithRecentEngagement )rank
INTO  #ETL_ActiveEloquacontacts
...
SELECT *
	, ROW_NUMBER() OVER (Partition By emailPrimary Order By ContactId DESC, LastEngagementDate DESC) rowNum
...
USING (SELECT *
		FROM etl.Eloqua_Contacts_Outbound_Stage
		WHERE rowNum = 1) AS mySource
