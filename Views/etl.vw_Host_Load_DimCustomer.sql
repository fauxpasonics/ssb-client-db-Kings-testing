...
CREATE VIEW [etl].[vw_Host_Load_DimCustomer] AS (
	...
	FROM (
		SELECT
			'Kings' AS [SourceDB]
			, 'Host Kings' AS [SourceSystem]
			, 1 AS [SourceSystemPriority]

			/*Standard Attributes*/
			, CAST(p.HostAccountId AS NVARCHAR(100)) [SSID]
			...
			, CAST (p.[STATE] AS NVARCHAR(200)) AS [AddressPrimaryState] 
			, CAST (p.ZIP AS NVARCHAR(25))	AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, CAST(p.DAYPHONE AS NVARCHAR(25)) AS [PhonePrimary]
			...
			/*Email*/
			, CAST(NULL AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, p.ETL__CreatedDate AS CreatedDate
			, p.ETL__UpdatedDate AS UpdatedDate
			, null AS IsDeleted
			, null AS DeleteDate
...
--		select top 100 *			
		FROM ods.Host_Accounts p		
		WHERE 1=1
		AND p.ETL__UpdatedDate > DATEADD(DAY,-3,GETDATE())
