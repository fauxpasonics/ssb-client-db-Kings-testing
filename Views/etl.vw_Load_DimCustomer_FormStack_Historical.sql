...
CREATE VIEW [etl].[vw_Load_DimCustomer_FormStack_Historical] AS 
...
            /*Name*/
            , NULL AS [Prefix]
            , z.Parent_guardian_name_first AS [FirstName]
            , NULL AS [MiddleName]
            , z.[Parent_Guardian_NameLast] AS [LastName]
            , NULL AS [Suffix]
            , NULL AS [Fullname]
            --, c.name_title as [Title]
 
            /*AddressPrimary*/
            , z.BillingAddress AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
            , z.BillingAddress2  AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
            , z.BillingCity AS [AddressPrimaryCity] 
            , z.BillingState  AS [AddressPrimaryState] 
            , z.BillingZip AS [AddressPrimaryZip] 
...
 
            /*Email*/
            , CAST([Parent_Guardian_Email] AS NVARCHAR(256)) AS [EmailPrimary]
...
            , CAST(z.Time AS DATETIME)AS [SSCreatedDate] --createddate
            , CAST(z.time AS DATETIME) AS [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT COUNT(*)           
        FROM stg.formstack_historical_files_Deduped z WITH (NOLOCK) --Change To Source Table
  	
    ) a
 
