...
CREATE VIEW [etl].[vw_Load_DimCustomer_Appetize] AS 
...
--base set
        SELECT
            'Kings' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
            , 'Appetize' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(email AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
...
 
            /*Phone*/
            , LEFT((CASE WHEN LEN(mobile) > 10 AND mobile LIKE '1%' THEN RIGHT(mobile, 10)
						 WHEN LEN(mobile) = 10 THEN mobile
						 ELSE NULL
						 END
						),25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
...
 
            /*Extended Attributes*/
            , Card_type AS[ExtAttribute1] --nvarchar(100)
...
            , Order_date AS [SSCreatedDate] --createddate
            , date_modified AS [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT COUNT(*)           
        FROM [ods].Appetize_Orders z WITH (NOLOCK) --Change To Source Table
        WHERE 1=1
		AND (z.email  NOT LIKE '%@appetize%' 
		AND z.email NOT LIKE  '%@aptize%'
		AND z.email <> '201Lowbrau2'
		AND z.email <> 'RMarino'
		AND z.email <> 'geneandmarylee.'
		AND z.email <> '201Lowbrau2'
		AND z.email <> 'mbryantzcutyofsacramento.org'
		AND z.email <> 'gtj'
		AND z.email <> 'q1`1'
		AND z.email <> '106sellands'
		AND z.email <> 'binm35'
		AND z.email <> 'thisisatestuser-12345'
		AND z.email <> 't'
		AND z.email <> 'g'
		AND z.email <> 'jylim87.'
		AND z.email <> 'valerie.peterson8gmail.com'
		AND z.email <> '203paragarys'
		AND z.email <> 'p115nacho1'
		AND z.email <> 'f'
		AND z.email <> 'p204beer1'
		AND z.email <> 'lars.wols.hpe.com'
		AND z.email <> '108bigcheese1'
		AND z.email <> '101bernardos1'
		AND z.email <> 'g1csuites31'
		AND z.email <> 'dzc z'
		AND z.email <> '212eljefe3'
		AND z.email <> 'ih '
		AND z.email <> 'thisisatestuser-123456789017'
		AND z.email <> '203paragarys1'
			)

    AND DATEDIFF(DAY,z.date_Modified,GETDATE())<=3 OR DATEDIFF(DAY,z.order_date,GETDATE())<=3 --This needs to be uncommented out and updated to the correct fields after the first load, update the date fields
    ) a
 
)
  
