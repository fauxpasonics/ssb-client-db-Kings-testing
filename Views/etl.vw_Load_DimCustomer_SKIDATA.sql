...
*****/
--drop view ods.vw_SKIDATA_LoadDimCustomer
CREATE VIEW [etl].[vw_Load_DimCustomer_SKIDATA] AS 
...
/*Name*/
      ...
      , NULL AS [Suffix]
      , sdu2.FirstName + ' ' + sdu2.LastName AS [FullName]
...
/*Source Created and Updated*/
      , NULL [SSCreatedBy]
      , NULL [SSUpdatedBy]
      , sdu.CreatedOnDate [SSCreatedDate]
      , sdu.CreatedOnDate [CreatedDate]
      , NULL [SSUpdatedDate]
      , sdu2.TicketAccountID [AccountId]
      , NULL IsBusiness
	  , 0 AS IsDeleted
	  , CASE WHEN sdu.ticketaccountID <>'' THEN CONCAT('TM', sdu.TicketAccountID) ELSE NULL END AS Customer_MatchKey
FROM    ods.SkiData_Users AS sdu
        JOIN dbo.vw_SKIDATA_UserProfileMap AS vsupm ON vsupm.UserID = sdu.UserID
        JOIN ods.SkiData_Users AS sdu2 ON sdu2.UserID = sdu.UserID
WHERE   sdu.ETL_UpdatedDate > DATEADD(DAY, -10, GETDATE())
                ) a
    );
