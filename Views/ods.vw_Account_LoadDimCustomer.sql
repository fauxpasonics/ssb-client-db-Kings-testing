...
CREATE VIEW [ods].[vw_Account_LoadDimCustomer] AS (
...
--base set
		...
		, a.Salutation AS [Prefix]
		, a.FirstName AS [FirstName]
		, NULL AS [MiddleName]
		, a.LastName AS [LastName]
		, CASE WHEN a.IspersonAccount = 1 THEN [Name] END AS [FullName]
		, NULL AS [Suffix]
		--, c.name_title as [Title]

		/*AddressPrimary*/
            , a.[BillingStreet] AS [AddressPrimaryStreet]
			, a.[BillingCity] AS [AddressPrimaryCity] 
			, a.[BillingState]	AS [AddressPrimaryState] 
			, a.[BillingPostalCode]	AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, a.[BillingCountry]	AS [AddressPrimaryCountry] 
			, [a].[BillingStreet] AS [AddressOneStreet]
			, [a].[BillingCity] AS [AddressOneCity] 
			, [a].[BillingState] AS [AddressOneState] 
			, [a].[BillingPostalCode] AS [AddressOneZip] 
			, NULL AS [AddressOneCounty] 
			, [a].[BillingCountry] AS [AddressOneCountry] 
			...
			, CASE WHEN isPersonAccount = 0 THEN NULL
					WHEN a.BillingStreet IS NULL AND a.[PersonEmail] IS NULL THEN CASE WHEN ISNUMERIC(a.Archtics_ID__C) = 1 THEN ISNULL(a.Archtics_ID__C, a.ID) ELSE a.ID END
