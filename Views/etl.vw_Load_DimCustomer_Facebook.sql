...
CREATE VIEW [etl].[vw_Load_DimCustomer_Facebook] AS 
...
            , 'Facebook' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(z.ID AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
...
            , z.zip_code AS [AddressPrimaryZip] 
...
 
            /*Phone*/
            , LEFT(CASE WHEN phone_number LIKE '+%' THEN SUBSTRING(phone_number,2,25)ELSE z.phone_number END,25) [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
...
            , z.created_time AS [SSCreatedDate] --createddate
            , z.created_time AS [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT COUNT(*)           
        FROM  [stg].[Facebook_Leads] z WITH (NOLOCK) --Change To Source Table
  	    WHERE 1=1
				AND z.recordcreatedate >= GETDATE()-3
				AND z.id NOT IN (
				'1486551918124463'
				,'370010513478957'
				,'2033671610234896'
